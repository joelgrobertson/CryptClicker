...
# Current loadout
var augment_slots: Array[Dictionary] = [] # Up to 4
var summon_slots: Array[Dictionary] = []  # Up to 3
var selected_summon_index: int = 0

# Has a hold-type augment been equipped?
...
var all_augments: Array[Dictionary] = [
	# Click augments
	{
		...
		"type": "click", "mana_per_proc": 3.0, "cooldown_clicks": 3,
	...
	},
	{
		...
		"type": "click", "mana_per_proc": 2.0, "cooldown_clicks": 4,
	...
	# Special
	{
		...
		"type": "hold_release", "mana_cost": 30.0, "charge_time": 1.5, "cooldown": 12.0,
...
var all_summons: Array[Dictionary] = [
	{
		"id": "skeleton_melee", "name": "Skeleton", "mana_cost": 5.0,
		"health": 40.0, "damage": 8.0, "speed": 50.0,
		"attack_cooldown": 1.2, "attack_range": 35.0, "patrol_radius": 40.0,
		"description": "Melee fighter",
		"color": Color(0.85, 0.85, 0.75),
		"level_4_desc": "Spawns with bone shield",
	},
	{
		"id": "skeleton_ranged", "name": "Sk. Archer", "mana_cost": 8.0,
		"health": 25.0, "damage": 6.0, "speed": 40.0,
		"attack_cooldown": 1.5, "attack_range": 150.0, "patrol_radius": 30.0,
		"description": "Ranged support",
		"color": Color(0.65, 0.72, 0.6),
		"level_4_desc": "Fires 2 arrows",
	},
	{
		"id": "goblin", "name": "Goblin", "mana_cost": 10.0,
		"health": 35.0, "damage": 7.0, "speed": 75.0,
		"attack_cooldown": 1.0, "attack_range": 35.0, "patrol_radius": 50.0,
		"description": "Fast attacker",
		"color": Color(0.3, 0.7, 0.3),
		"level_4_desc": "30% dodge chance",
	},
	{
		"id": "zombie", "name": "Zombie", "mana_cost": 7.0,
		"health": 80.0, "damage": 5.0, "speed": 30.0,
		"attack_cooldown": 1.8, "attack_range": 30.0, "patrol_radius": 25.0,
		"description": "Tanky meat wall",
		"color": Color(0.4, 0.5, 0.35),
		"level_4_desc": "Thorns: attackers take damage",
	},
	{
		"id": "ghoul", "name": "Ghoul", "mana_cost": 15.0,
		"health": 45.0, "damage": 6.0, "speed": 55.0,
		"attack_cooldown": 1.3, "attack_range": 40.0, "patrol_radius": 35.0,
		"description": "Poisons enemies",
		"color": Color(0.5, 0.35, 0.5),
		"level_4_desc": "Poison aura damages nearby",
	},
	{
		"id": "imp", "name": "Imp", "mana_cost": 12.0,
		"health": 30.0, "damage": 10.0, "speed": 45.0,
		"attack_cooldown": 1.6, "attack_range": 120.0, "patrol_radius": 30.0,
		"description": "Ranged fire DPS",
		"color": Color(0.9, 0.3, 0.2),
		"level_4_desc": "Fireballs splash AOE",
	},
	{
		"id": "wizard", "name": "Wizard", "mana_cost": 20.0,
		"health": 20.0, "damage": 15.0, "speed": 30.0,
		"attack_cooldown": 2.0, "attack_range": 200.0, "patrol_radius": 20.0,
...
func generate_picks(count: int = 3, force_summon: bool = false) -> Array[Dictionary]:
	...
	# New augments (if slots available)
	if augment_slots.size() < MAX_AUGMENT_SLOTS:
		for aug in all_augments:
			if has_augment(aug["id"]):
				continue
			# Don't offer hold augments if we already have one
			if aug.get("type", "") in ["hold", "hold_release"] and has_hold_augment:
				continue
			candidates.append({
				"pick_type": "new_augment",
				"tool": aug,
				"weight": 3.0,
				"display_name": aug["name"],
				"display_desc": aug["description"],
				"display_color": aug.get("color", Color.WHITE),
				"display_tag": "NEW AUGMENT",
			})
	
	# New summons (if slots available)
	if summon_slots.size() < MAX_SUMMON_SLOTS:
		for summon in all_summons:
			...
			var w = 3.0
			if force_summon:
				w = 10.0 # Heavily favor summons for first pick
			candidates.append({
				"pick_type": "new_summon",
				"tool": summon,
				"weight": w,
				"display_name": summon["name"],
				"display_desc": summon["description"],
				"display_color": summon.get("color", Color.WHITE),
				"display_tag": "NEW SUMMON",
			})
	
	# Level-ups for owned tools
	for aug in augment_slots:
		...
		if lvl < MAX_TOOL_LEVEL:
			...
			if lvl + 1 == 3:
				...
				desc += ": " + aug.get("level_4_desc", "Special bonus")
			elif lvl + 1 == 5:
				desc += " (MAX)"
			candidates.append({
				"pick_type": "level_up",
	...
	for summon in summon_slots:
		...
		if lvl < MAX_TOOL_LEVEL:
			...
			if lvl + 1 == 3:
				...
				desc += ": " + summon.get("level_4_desc", "Special bonus")
			elif lvl + 1 == 5:
				desc += " (MAX)"
			candidates.append({
				"pick_type": "level_up",
	...
	# Fallback bonus options
	if candidates.size() < count:
		candidates.append({
			"pick_type": "bonus", "bonus_id": "mana_burst",
			"weight": 1.0, "display_name": "Mana Burst",
			"display_desc": "Instantly refill 50 mana",
			"display_color": Color(0.3, 0.5, 1.0),
			"display_tag": "BONUS",
		})
		candidates.append({
			"pick_type": "bonus", "bonus_id": "gold_pile",
			"weight": 1.0, "display_name": "Gold Pile",
			"display_desc": "Gain 50 gold",
			"display_color": Color(1.0, 0.85, 0.1),
			"display_tag": "BONUS",
		})
	...
	# Ensure at least 1 summon on first pick if forced
	if force_summon and picks.size() > 0:
		...
		if not has_summon_pick:
			# Find a summon candidate and swap it in
			for c in candidates:
...
func apply_pick(pick: Dictionary):
	match pick["pick_type"]:
		"new_augment":
			add_augment(pick["tool"])
		"new_summon":
			add_summon(pick["tool"])
		"level_up":
			level_up_tool(pick["tool_id"])
		"bonus":
			_apply_bonus(pick.get("bonus_id", ""))

func _apply_bonus(bonus_id: String):
	match bonus_id:
		"mana_burst":
			GameManager.add_mana(50.0)
		"gold_pile":
